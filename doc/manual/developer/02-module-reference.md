...
| `face_store` | `src/face_store.rs` | *Feature-gated (`ai`)*. SQLite-backed storage for face detections and named people. Tables: `faces(id, asset_id, person_id, bbox_*, confidence, embedding, crop_path, created_at)`, `people(id, name, representative_face_id, created_at)`. Provides `FaceStore` with `insert_face()`, `faces_for_asset()`, `create_person()`, `name_person()`, `assign_face()`, `unassign_face()`, `merge_people()`, `delete_person()`, `auto_cluster()` (greedy single-linkage clustering by cosine similarity), `list_people()`, `update_face_count()` (denormalized column). Structs: `StoredFace`, `Person`, `AutoClusterResult`, `FaceMatch`. |
